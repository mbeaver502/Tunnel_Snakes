...
namespace Assist_UNA
{
    class Assembler
    {
        ...
        *****************************************************************************************/
        ...
        public string Pass1()
        {
            ...
            if ( line != null && line.StartsWith("$ENTRY"))
            {
                ...
                if (inStream.EndOfStream && line.TrimEnd() != "$ENTRY")
                {
                    ...
                    /*
                     * ====================================================================
                     * THIS NEEDS TO WORK WITH ABSOLUTE PATHS AND RELATIVE PATHS,
                ...
                if (line != "" && line[0] != '#')
                {
                    ...
                    else
                    {
                                ...
                                */
                                ...
                                case "END":
                                    ...
                                    prtOutStream.WriteLine(String.Format("{0,6} {1,4} {2,4} {3}",
                                                                     ...
                                                                     " ", line));
                                    numLinesPrintedPerPage++;
                                    if (!AddPagePRT(pageNumber, prtOutStream))
                                    ...
                                    for (int i = 0; i < tempLiterals.Length; i++)
                                    {
                                        ...
                                        line = String.Format("{0,6} {1,6} {2}", " ", " ", tempLiterals[i]);
                                ...
                                case "LTORG":
                                    ...
                                    for (int i = 0; i < tempLiterals.Length; i++)
                                    {
                                        ...
                                        line = String.Format("{0,6} {1,8} {2}", " ", " ", tempLiterals[i]);

                                        prtOutStream.WriteLine("{0,6} {1,-14} {2}", tempAddresses[i],
                                                               objCode1, line);
                                        numLinesPrintedPerPage++;
                                        tempLineNumber++;
                        ...
                        else
                        {
                            ...
                            {
                                intermediateObjCodeSize = Convert.ToInt32(tempIntermediateLine[0], 16) * 2;
                                ...
                                else if (intermediateObjCodeSize <= 8)
                                {
                                    ...
                                    objCode2 = objCode.Substring(locationVal + 4, intermediateObjCodeSize - 4);
                                }
                                ...
                                else if (intermediateObjCodeSize <= 12)
                                {
                                    ...
                                    objCode3 = objCode.Substring(locationVal + 8, intermediateObjCodeSize - 8);
                                }
                            }                            
         ...
         *              
         *****************************************************************************************/
        private bool AddPagePRT(int pageNumber, StreamWriter prtOutStream)
        {
                            ...
                            case "RX":
                                ...
                                else if (intermediateInstructionOpCode == "47")
                                {
                                    ...
                                    if (symTable.IsSymbol(intermediateParam1))
                                    {
                                        ...
                                        displacement2 =
                                            Convert.ToInt32(symTable.GetAddress(intermediateParam1), 16);
                                        displacement2 -= Convert.ToInt32(baseRegisterContents, 16);
                                    }

                                    /* The second parameter is in D(,B) or D(L,B) format. */
                                    else if (intermediateParam1.Contains("(") && intermediateParam1.Contains(","))
                                    {
                                        ...
                                        displacement2 = Convert.ToInt32(intermediateParam1.Substring(0, openParen));

                                        if (openParen == commaIndex - 1)
                                            index1 = 0;
                                        else
                                            index1 = Convert.ToInt32(intermediateParam1.Substring(openParen + 1, commaIndex - openParen - 1));

                                        base2 = Convert.ToInt32(intermediateParam1.Substring(commaIndex + 1, closeParen - commaIndex - 1));
                                    }
                                ...
                                if (intermediateParam2 != "")
                                {
                                    ...
                                    else if (literalPools[numLiteralPools].IsLiteral(intermediateParam2.TrimEnd()))
                                    {
                                        ...
                                        displacement2 =
                                            Convert.ToInt32(literalPools[numLiteralPools].GetAddress(intermediateParam2), 16);
                                        displacement2 -= Convert.ToInt32(baseRegisterContents, 16);
                                    }
...
*/
                    ...
                    */
                    if (instruction != "TITLE" && instruction != "SPACE" && instruction != "EJECT")
                    {
                        ...
                        {
                            //parameterList[0] = parameters;

                            /* Just get the parameters in one entry of array. */
                            parameterList = parameters.Split('!');
                            //string[] tempArray = new string[1];
                            //Array.Copy(parameterList, tempArray, 1);
            ...
            //catch (ExceededMaxInstructionException)
            //{
            //    MessageBox.Show(String.Format(
            //        "The program has exceeded the max number ({0}) of instructions.",
            //        optionsInstructions), "Error - Exceeded Max Instructions");
            //    return;
            //}
            ...
            finally
            {
                if (neededForIntermediate)
                {
                    ...
                    if (parameterList == null)
                        intermediateOutStream.WriteLine("{0}|{1}|{2}|{3}|{4}", increment, (lineNumber + 1), locationCounterHex, label, instruction.TrimEnd());
                    else if (parameterList.Length == 1 && parameterList[0] != "")
                        intermediateOutStream.WriteLine("{0}|{1}|{2}|{3}|{4}|{5}", increment, (lineNumber + 1), locationCounterHex, label, instruction.TrimEnd(),
                                                        parameterList[0].TrimEnd());
                    else if (parameterList.Length == 2 && parameterList[0] != "")
                        intermediateOutStream.WriteLine("{0}|{1}|{2}|{3}|{4}|{5}|{6}", increment, (lineNumber + 1), locationCounterHex, label, instruction.TrimEnd(),
                                                        parameterList[0].TrimEnd(), parameterList[1].TrimEnd());
                    else if (parameterList.Length == 3 && parameterList[0] != "" && parameterList[1] != "" && parameterList[2] != "")
                        intermediateOutStream.WriteLine("{0}|{1}|{2}|{3}|{4}|{5}|{6}|{7}", increment, (lineNumber + 1), locationCounterHex, label, instruction.TrimEnd(),
                                                        parameterList[0].TrimEnd(), parameterList[1].TrimEnd(),
                ...
                foreach (string literal in literals)
                {
                    ...
                    switch (type)
                    {
                        ...
                        case "=P":
                            ...
                            currentAddress += (tempLength / 2);
                            break;

                        case "=X":
                            if (literal.Length % 2 == 1)
                                currentAddress += ((literal.Length - 3) / 2);
                            else
                        ...
                        case "=H":
                            ...
                            literalPools[numLiteralPools].UpdateAddress(literal, currentAddress.ToString("X").PadLeft(6, '0'));
        ...
        private bool ValidateLabel()
        {
            ...
            {
                MessageBox.Show("Max number of symbols exceeded, assembly terminated.", 
                                "Error - Exceeded Max Symbols");
                return false;
            }
        }
        ...
        private bool ValidateParameters()
        {
            ...
            {
                MessageBox.Show("Max number of literals ({0}) exceeded, assembly terminated.",
